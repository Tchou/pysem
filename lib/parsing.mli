...
(** Informations about an identifier: scope, context and locations of its
    occurrences. *)
...
module BlockId : sig
  type t
  val mk : kind:block_kind -> name:string ->
           location:PyreAst.Concrete.Location.t -> t
...
type block_info =
  { name : string
  (** For a module, the filename, for a lambda the string ["<LAMBDA>"]. *)
  ; filename : string (** filename *)
  ; location : PyreAst.Concrete.Location.t
  (** Location of the block in the file. *)
  ; kind : block_kind (** The kind of the block. *)
  ; identifiers : info IdentMap.t
  (** A map of identifers defined in the scope of the block. *)
  ; defines : (string * PyreAst.Concrete.Location.t * block_kind) list
  (** Name, location and kind of the blocks defined in this one. *)
  }
...
val pp_vars : Format.formatter -> (PyreAst.Concrete.Identifier.t * info) list ->
              unit

val parse : file:string ->
            PyreAst.Concrete.Module.t * block_info list * Utils.loc_converter
(** [parse ~file] returns a pair [(ast, bil)] where [ast] is the concrete AST of
    the module defined written in [file] and [bil] is the list of all scope
    blocks defined in the file.
