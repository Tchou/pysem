...
let builtins = (* Available via [dirs(__builtins__)] *)
  ...
  ; "Warning"; "ZeroDivisionError"; "_IncompleteInputError"; "__build_class__"
  ...
  let to_pyre e =
    ...
    | IncompatibleScope (id1, loc, s1, s2) ->
       mk_error (sprintf "Name '%s' has incompatible scope %s and %s"
         ...
         loc
    | AltPatternNames loc ->
       mk_error "Alternative pattern binds different names" loc
...
(* internal errors, re-raised see parse at the end of the file *)
...
let ident ?location ?(scope=Unknown)
      ?(ctx=PyCo.ExpressionContext.make_load_of_t()) id =
  IdentMap.singleton id
    { scope; context = context ctx; locations = Option.to_list location }
...
module BlockId = struct
  ...
  let mk ~kind ~name ~location =
    { kind; name=PyCo.Identifier.make_t name (); location }
...
let enter_arguments scope (a : PyCo.Arguments.t) vars =
  ...
  let add_arg_list l vars =
    List.fold_left (fun avars PyCo.Argument.{location;identifier;_} ->
        ...
        then raise_ (DuplicateArgument (identifier, location));
...
(* Function declarations introduces a new block (and hence a new scope).
   [compute_fun_variables] closes the scope corresponding to the body of the
   function
 *)
...
let compute_block_variables env location kind name args body =
  (* vars is the mapping from variable names to info.
     ...
     - if it is unknown but has a stored or del context, then make it local to
       the current scope
     - if it is local and the kind is 'Module' make it global.
     Then:
     - all variables that are still unknown are marked as free variables and
     returned
   *)
  let vars =
    List.fold_left (fun acc (_,v, _) -> merge_vars acc v) IdentMap.empty body in
  let idents = List.concat_map get3 body in
  let vars =
    enter_arguments (if kind = Module then Global else Parameter) args vars in
  ...
  let nvars =
    vars
    |> IdentMap.filter_map (fun var info ->
           let scope =
             ...
             | s -> if IdentTable.mem env.free var
                    then todo := (var, kind = Module)::!todo;
                    s
           in
  ...
  let rem_vars = if PyCo.Identifier.compare name lambda_id = 0 || kind = Module
                 then rem_vars
                 else (* functions bind their name *)
                   merge_vars rem_vars (ident ~location ~ctx:store_ctx name) in
...
let ( let* ) (a, (vars:info IdentMap.t), (l : BlockId.t list)) f =
  (f a), vars, l
...
let constant =
  ...
  PyTF.Constant.make ~none:(make_none_of_t ()) ~false_:(make_false_of_t ())
    ~true_:(make_true_of_t ()) ~ellipsis:(make_ellipsis_of_t ())
    ~integer:make_integer_of_t ~big_integer:make_big_integer_of_t
    ~float_:make_float_of_t ~complex:make_complex_of_t ~string_:make_string_of_t
    ~byte_string:make_byte_string_of_t ()

let expression_context =
  let open PyCo.ExpressionContext in
  PyTF.ExpressionContext.make ~load:(make_load_of_t ())
    ~store:(make_store_of_t ()) ~del:(make_del_of_t ()) ()
...
let binary_operator =
  ...
  PyTF.BinaryOperator.make ~add:(make_add_of_t ()) ~sub:(make_sub_of_t ())
    ~mult:(make_mult_of_t ()) ~matmult:(make_matmult_of_t ())
    ~div:(make_div_of_t ()) ~mod_:(make_mod_of_t ()) ~pow:(make_pow_of_t ())
    ~lshift:(make_lshift_of_t ()) ~rshift:(make_rshift_of_t ())
    ~bitor:(make_bitor_of_t ()) ~bitxor:(make_bitxor_of_t ())
...
let keyword ~location ~arg ~value =
  (* The arg identifier in a keyword is not binding, and does not interact with
     the namespace
     def f():
       global x
...
let arguments ~posonlyargs ~args ~vararg ~kwonlyargs ~kw_defaults ~kwarg
      ~defaults =
  let* _init and*@ posonlyargs
  and*@ args and*? vararg
  and*@ kwonlyargs and*?@ kw_defaults
  and*? kwarg and*@ defaults in
  PyCo.Arguments.make_t ~posonlyargs ~args ?vararg ~kwonlyargs ~kw_defaults
    ?kwarg ~defaults ()

(* Expressions: auxiliary definitions corresponding to constructors of the
   Concrete.Expressioon.t are local functions *)
...
let expression tbl =
  ...
  let named_expr ~location ~target ~value =
    (* target := expr. target is guaranteed to be a variable name. Nothing
       special to do here, the name case below handles the target, for which the
       parser will put the Store context *)
  ...
  let lambda ~location ~args ~body =
    let* _init and* args
    and* body = compute_block_variables tbl location Lambda lambda_id
                  (get1 args) [body] in
  ...
  let attribute ~location ~value ~attr ~ctx =
    (* Here the context is for the field access [a.b] itself, so pass it
       along. *)
    let* value in
  ...
  in
  PyTF.Expression.make ~bool_op ~named_expr ~bin_op ~unary_op ~lambda ~if_exp
    ~dict ~set ~list_comp ~set_comp ~dict_comp ~generator_exp ~await ~yield
    ~yield_from ~compare ~call ~formatted_value ~joined_str ~constant ~attribute
...
let pattern =
  ...
  let match_mapping ~location ~keys ~patterns ~rest =
    (* keys are guaranteed to be constants so there should be no identifier with
       a del or store context inside *)
  ...
  let match_as ~location ~pattern ~name =
    let* _init
    and*? pattern
    and* name = bind_opt ~location name in
    (* case _ is None, None, case x: is None, Some x, case x as _ is rejected by
       the parser *)
  ...
  let match_or ~location ~patterns =
    let () = (* detect patterns with bad domain *)
      ...
      | (_,v,_) :: l ->
         ...
         then raise_ (AltPatternNames location)
    in
    let* _init and*@ patterns in
    make_matchor_of_t ~location ~patterns ()
  in
  PyTF.Pattern.make ~match_value ~match_singleton ~match_sequence ~match_mapping
    ~match_class ~match_star ~match_as ~match_or ()

(* Statements *)
let statement tbl =
  let open PyCo.Statement in
  let mk_fun mk kind ~location ~name ~args ~body ~decorator_list ~returns
    ...
    and*? returns
    and*@ type_params in
    mk ~location ~name ~args ?body:(Some body)
      ?decorator_list:(Some decorator_list) ?returns ?type_comment
  ...
  let class_def ~location ~name ~bases ~keywords ~body ~decorator_list
    ...
    and*@ bases
    and*@ keywords
    and* body = compute_block_variables tbl location Class name
  ...
  let mk_for mk ~location ~target ~iter ~body ~orelse ~type_comment =
    ...
    mk ~location ~target ~iter ?body:(Some body) ?orelse:(Some orelse)
      ?type_comment ()
  in
  ...
  let mk_try mk ~location ~body ~handlers ~orelse ~finalbody =
    ...
    mk ~location ?body:(Some body) ?handlers:(Some handlers)
      ?orelse:(Some orelse) ?finalbody:(Some finalbody) ()
  in
  ...
  let mk_scope mk scope ~location ~names =
    let vars = List.fold_left (fun acc n ->
                   IdentMap.add n
                     {scope; context=default_context;locations=[location]} acc)
  ...
  let expr ~location ~value =
    let* value in
    make_expr_of_t ~location ~value ()
  in
  PyTF.Statement.make ~function_def ~async_function_def ~class_def ~return
    ~delete ~assign ~type_alias ~aug_assign ~ann_assign ~for_ ~async_for ~while_
    ~if_ ~with_ ~async_with ~match_ ~raise_ ~try_ ~try_star ~assert_ ~import
    ~import_from ~global ~nonlocal ~expr
...
let function_type ~argtypes ~returns =
  ...
  and*@ argtypes
  and* returns in
  PyCo.FunctionType.make_t ~argtypes ~returns ()
...
let pp_vars fmt vars =
  ...
  fprintf fmt "%a"
    (pp_print_list ~pp_sep:pp_print_space
       (fun fmt (v, i) -> fprintf fmt "%s=%a"
                            (PyCo.Identifier.to_string v) pp_info i))
    vars
...
let rec make_summary (tbl : env) bid =
  ...
  ; defines =
      List.map (fun bid ->
          BlockId.(PyCo.Identifier.to_string bid.name, bid.location, bid.kind))
        bids;
...
let module_gen (tbl:env) ~body ~type_ignores =
  ...
  let body, vars, bids =
    compute_block_variables tbl dummy_loc Module
      module_name
...
let module_ (tbl:env) ~body ~type_ignores =
  ...
  | Some (ident, {locations=loc::_;_}) ->
     raise_ (UndefinedGlobalName(ident, loc))
  | Some (ident, {locations=[];_}) ->
     raise_ (UndefinedGlobalName(ident, dummy_loc))
  | None ->
     m,make_summary_list tbl i

(* after we are done, any remaining variable that has scope Local is changed to
   Global (it is "local" to the module) and any free variable that remains
   should raise an error.
 *)

let spec env =
  PyTF.make ~argument ~arguments ~binary_operator ~boolean_operator
    ~comparison_operator ~comprehension ~constant ~exception_handler
    ~expression:(expression env) ~expression_context ~function_type
    ~identifier:(fun s -> PyCo.Identifier.make_t s ()) ~import_alias ~keyword
    ~location:(fun ~start ~stop -> PyCo.Location.make_t ~start ~stop ())
    ~match_case ~module_:(module_ env) ~pattern
    ~position:(fun ~line ~column -> PyCo.Position.make_t ~line ~column ())
    ~statement:(statement env) ~type_ignore ~type_param ~unary_operator
    ~with_item ()
...
let parse ~file =
  ...
  match
    Parser.with_context (fun context ->
        Parser.
        TaglessFinal.parse_module ~context ~spec:(spec env)
