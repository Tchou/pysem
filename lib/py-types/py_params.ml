...
let ast_get_field_or_default field =
  ...
  let args = [ mk_rec_disj true [[ (field, (tv, true)) ]]
             ; tv ]
             |> MT.Tuple.mk in
...
let generic_getter p field_name arg odef =
  ...
  | None -> (* perform arg.field_name *)
    Utils.mk_projection p (MSAst.PiField field_name) arg
...
let ast_get_field2 (field1, field2) =
  ...
  let args =
    [ mk_rec_disj true
        [[ (field1, (tv, false)); (field2, (Sstt.Ty.empty, true)) ]]
    ; mk_rec_disj true
        [[ (field2, (tv, false)); (field1, (Sstt.Ty.empty, true)) ]] ]
    |> MT.Ty.disj
  in
  let fty = MT.Arrow.mk args tv |> MlGTy.mk in
  mk_value dummy_pos fty
...
let ast_get_field2_def (field1, field2) =
  ...
  let args =
    [ [ mk_rec_disj true
          [[ (field1, (tv, false)); (field2, (Ty.empty, true)) ]]; Ty.any ]
    ; [ mk_rec_disj true
          [[ (field2, (tv, false)); (field1, (Ty.empty, true)) ]]; Ty.any]
    ; [ mk_rec_disj true
          [[ (field2, (Ty.empty, true)); (field1, (Ty.empty, true)) ]]; tv] ]
...
let is_non_empty_rec r =
  ...
  && (MT.TVOp.top_vars r |> MT.MVarSet.is_empty)
...
let to_approx builder rec_type =
  let open Sstt in
  let sigs =
    Op.Records.as_union (rec_type |> Ty.get_descr |> Descr.get_records) in
  let b = List.fold_left (fun acc a ->
      Ty.cup acc (a |> Op.Records.of_atom |> Descr.mk_records |>Ty.mk_descr))
      Ty.empty sigs
          |> Ty.equiv rec_type
  in
  Approx (b, List.map (record_atom_to_sig builder) sigs)

let field_param builder is_anon rec_type p =
  let field =
    if is_anon
    then field_name_pos p.pos
    else field_name_kwd p.name
  in
  MT.Record.proj rec_type field |> builder
...
let to_exact builder rec_type spec_id =
  let descr = EnumHash.find descr_by_id spec_id in
  let tr_kind = function
    | Str _ as s -> s
    | Anon p -> Anon { p with ty = field_param builder true rec_type p }
    | Named p -> Named { p with ty = field_param builder false rec_type p }
  in
...
let to_t (ctx : Sstt.Printer.build_ctx) comp =
  let open Sstt in
  ...
  | None -> Some (to_approx ctx.build rec_type)
  | Some id -> Some (to_exact ctx.build rec_type id)
...
let pp_py_scheme fmt s =
  ...
  let vars = MT.MVarSet.elements1 vars in
